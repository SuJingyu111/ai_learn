...
完成后可以在 [`PROGRESS.md`](PROGRESS.md) 勾选对应的 Step。

「你应该看到」分两种，步骤里会写明是哪一种：

- **逐位一致**——手算出来的值、shape、strides、offset。对不上就是有 bug，
  先解决再往下走。
- **量级或趋势一致**——内存地址、耗时、浮点误差。这些依赖你的机器、
  编译器和随机种子，不可能逐位相同；要对上的是**关系**
  （两个地址必须相等、p95 必须大于 median、误差必须随 K 按 sqrt 增长）。

把第二类当第一类去追会浪费很多时间；把第一类当第二类放过则会让 bug 溜进下一步。
